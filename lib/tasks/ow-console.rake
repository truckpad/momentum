namespace :ow do
  ...
  task :console, [:to, :env, :aws_id, :aws_secret] do |t, args|
    ...
    command = "'sudo su deploy -c \"source #{Momentum.config[:source_app_env]}; cd /srv/www/#{Momentum.config[:app_short_name]}/current && RAILS_ENV=#{args[:env] || args[:to]} bundle exec rails console\"'"
